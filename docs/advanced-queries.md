...
```python
>>> posts_q = Post.select(Post.id, Post.title).aliased("post")
>>> q = User.select(User.id, User.name).prefetch(posts_q)
>>> q.build()
('SELECT "user"."id","user"."name",(SELECT COALESCE(json_agg(t),\'[]\'::json) FROM (SELECT "post"."id","post"."title" FROM "public"."post") t) AS "post" FROM "public"."user"', ())
