...
async def test_unique_and_index_make_and_apply(pg_conn: Any, tmp_path: Path) -> None:
    await pg_conn.execute("DROP TABLE IF EXISTS public.norm_ci_widgets")
    ...
    assert (
        'AddConstraint(table="norm_ci_widgets", '
        'constraint={"type": "unique", "name": "norm_ci_widgets_email_key", '
        '"columns": ("email",)}, schema="public"),'
    ) in body
    assert (
        'CreateIndex(table="norm_ci_widgets", columns=("score",), '
        'name="idx_norm_ci_widgets_score", method=None, unique=False, '
