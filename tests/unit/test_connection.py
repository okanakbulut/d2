...
def driver() -> MagicMock:
    mock = MagicMock()
    mock.set_type_codec = AsyncMock()
    return mock


class TestFetch:
    def test_single_returns_struct_when_row_exists(self, driver: MagicMock):
    ...
    def test_single_returns_none_when_no_row(self, driver: MagicMock):
    ...
    def test_single_calls_fetchrow_with_correct_sql_and_params(self, driver: MagicMock):
        driver.fetchrow = AsyncMock(return_value=None)
        conn = AsyncConnection(driver)
    ...
    def test_many_returns_list_of_structs(self, driver: MagicMock):
    ...
    def test_many_calls_fetch_with_correct_sql(self, driver: MagicMock):
...
class TestExecute:
    def test_returns_driver_status(self, driver: MagicMock):
    ...
    def test_calls_driver_with_correct_sql_and_params(self, driver: MagicMock):
        driver.execute = AsyncMock(return_value="INSERT 0 1")
        conn = AsyncConnection(driver)
...
class TestExecuteMany:
    def test_calls_executemany_with_param_list(self, driver: MagicMock):
...
class TestTransaction:
    def test_returns_driver_context_manager(self, driver: MagicMock):
        conn = AsyncConnection(driver)
