...
from norm.query import SubqueryProxy
from .conftest import Users, Posts, Orders, Employees

class TestNamedCTE:
    @pytest.fixture
    def rev(self) -> SubqueryProxy:
        return (
    ...
    def test_single_named_cte_sql(self):
        recent_posts = Posts.select(Posts.user_id, Posts.title).as_view("recent_posts")
        q = With(
            recent_posts,
            query=(
    ...
    def test_cte_params_numbered_before_outer_params(self):
        recent_posts = Posts.select(Posts.user_id, Posts.title).where(Posts.id > 100).as_view("recent_posts")
        q = With(
            recent_posts,
            query=(
    ...
    def test_left_join_with_named_cte(self, rev: SubqueryProxy):
        q = With(
            rev,
            query=(
    ...
    def test_cte_column_in_outer_where(self, rev: SubqueryProxy):
        q = With(
            rev,
            query=(
                Users.select(Users.name)
    ...
    def test_two_ctes_rendered_in_registration_order(self):
        cte1 = Posts.select(Posts.user_id, Posts.title).as_view("cte1")
        cte2 = Orders.select(Orders.user_id, Orders.amount).as_view("cte2")
